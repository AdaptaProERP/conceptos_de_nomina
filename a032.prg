//Incidencia por Jorada Mixta Diurna (H)
FUNCTION A032(uPar1,uPar2,uPar3,uPar4,uPar5)
LOCAL nResult:=0, SD:=0, SXH:=0, BNOCT:=0,SNXH:=0,SXH2:=0,SNXH2:=0,IMIXTA:=0
LOCAL nResult:=0,SD:=0,SXH:=0,BNOCT:=0,SNXH:=0,SXH2:=0,SNXH2:=0,SXH1:=0,SXH3:=0,SXH4:=0
LOCAL nDias, nDiasIna:=0 , nHoras:=0
...
IF CONDICION=[A] .AND. TIPO_NOM="S" .AND. TURNO="MIX"
...
//---------- Calculo Salario Mixto Mensual Preliminar  -----------------------------
     //VARIAC:=(ndias*4.5)+(ndias*3)   //nHoras*nDias     //     VARIAC:=135
     //nResult:=SMD1-(IMIXTA*VARIAC+SALARIO)
     VARIAC:=(30*7.5)
     nResult:=SMD1-(IMIXTA*VARIAC+(Salario))
     nResult:=(nresult/30)*ndias
     VARIAC:=NDIAS
     //nResult:=IMIXTA*VARIAC+SALARIO
ENDIF
//===============================================================================
RETURN nResult
// EOF
